...
export default (emitter) => {
	...
	gl.init().then(() => {
		...
		console.warn('Environment map init failed:', err)
	})
	...
	useGLTF(`/webgl/plate-fitcity-updated.glb`).then((e) => {
		const plate = e.scene.getObjectByName('plate_updated')
		...
		// FITCITY text - single mesh, single material, transforms pre-applied
		const fitcityText = e.scene.getObjectByName('fitcity_text')
		if (fitcityText) {
			textMesh = new InstancedMesh(fitcityText.geometry, fitcityText.material, items.length)
		...
		for (let i = 0; i < instancedMesh.count; i++) {
			...
			if (textMesh) textMesh.setMatrixAt(i, dummy.matrix)
		}
		...
		if (textMesh) textMesh.instanceMatrix.needsUpdate = true
		...
		gl.init().then(() => {
			...
			console.error('WebGL render init failed:', err)
	...
	useFrame(({ timer }) => {
		...
		items.forEach((it, index) => {
			if (it.pointerdown === true) {
				...
				if (textMesh) textMesh.setMatrixAt(currentIndex, instanceObject.matrix)
				instancedMesh.instanceMatrix.needsUpdate = true
				ironMesh.instanceMatrix.needsUpdate = true
				if (textMesh) textMesh.instanceMatrix.needsUpdate = true
			}
