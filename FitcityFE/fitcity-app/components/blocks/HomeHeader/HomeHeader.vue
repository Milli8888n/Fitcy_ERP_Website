...
const close = (slide = null) => {
	...
	nextTick(() => {
		setTimeout(() => {
			...
			// Start menu theme and background transition slightly later 
			// to match the peak of the curtain/ribbon close
			setTimeout(() => {
...
const changeMenuTheme = (nextSlide) => {
	...
	const themeIndex = (nextSlide % 3) + 1
	let theme
	switch (themeIndex) {
		case 1:
...
onMounted(() => {
	...
	context.$page.loader.ready.then(() => {
		initRibbon()
		init() // Moved from transitionReady to start earlier
		if (bullets.value) {
...
[data-component='home-header'] {
	...
	.contents-container {
		...
		.ribbon {
			...
			// Colors repeat every 3 elements
			&:nth-child(3n + 1) {
				--ribbon-backgroundColor: #FF5800; /* Electric Orange */
				--track-color: #000;
				color: var(--track-color);
			}
			&:nth-child(3n + 2) {
				--ribbon-backgroundColor: #C4D600; /* Cyber Lime */
				--track-color: #004225;
				color: var(--track-color);
			}
			&:nth-child(3n + 3) {
				--ribbon-backgroundColor: #D9E1E2; /* Arctic White */
				--track-color: #FF5800;
				color: var(--track-color);
			}
		}
		...
		.background {
			...
			// Colors repeat every 3 elements
			&:nth-child(3n + 1) {
				--ribbon-backgroundColor: #FF5800;
			}
			&:nth-child(3n + 2) {
				--ribbon-backgroundColor: #C4D600;
			}
			&:nth-child(3n + 3) {
				--ribbon-backgroundColor: #D9E1E2;
			}
		}
